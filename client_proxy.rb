...
class Client_Proxy
  ...
  def directory_server(filename, msg)
    ...
    ds.puts(@ticket)
    ds.puts(encrypt(msg % filename, @session_key))
  ...
  def open(filename)
    address = directory_server(filename, FIND_SERVER)
    if address == NOT_FOUND
      ...
      fs.puts(@ticket)
      fs.puts(encrypt(OPEN_FILE % filename, @session_key))
